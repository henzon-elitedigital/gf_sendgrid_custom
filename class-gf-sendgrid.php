...
class GF_SendGrid extends GFAddOn {
	 ...
	 */
	protected $_title = 'Customized Gravity Forms SendGrid Add-On';
	 ...
	 */
	protected $_short_title = 'Custom SendGrid';
		...
		return array(
			array(
				...
				'fields' => array(
					array(
						'name'              => 'apiEndpoint',
						'label'             => esc_html__( 'Custom Endpoint', 'gravityformssendgrid' ),
						'type'              => 'text',
						'class'             => 'large',
	...
	public function maybe_send_email( $email, $message_format, $notification, $entry ) {
		...
		// if ( 'sendgrid' !== rgar( $notification, 'service' ) ) {
		...
		try {
			...
			// insert custom endpoint if configured
			if (null !== $this->get_plugin_setting( 'apiEndpoint' ) && !empty($this->get_plugin_setting( 'apiEndpoint' ))) {
				$sendgrid_email['custom_endpoint'] = $this->get_plugin_setting( 'apiEndpoint' );
			}
